...
export const bookTour = async (tourId,startDateId) =>{
    try{
        console.log("hello")
        //1Create tour
        const product = await axios.post(`http://localhost:3000/api/v1/bookings/create-product/${tourId}`);
        console.log(product);
            if(!product) return new AppError("Couldn't create product!Please try again")
              
        
        //Create price
        const price = await axios.post(`http://localhost:3000/api/v1/bookings/create-price/${tourId}/${product.data.product.id}`);
        ...
        const session = await axios(`http://localhost:3000/api/v1/bookings/checkout-session/${tourId}/${price.data.price.id}/${startDateId}`);
